...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, copy) void (^ _Nullable onUserPurchase)(void);)
+ (void (^ _Nullable)(void))onUserPurchase SWIFT_WARN_UNUSED_RESULT;
+ (void)setOnUserPurchase:(void (^ _Nullable)(void))value;
...
+ (void)onUserPurchaseSuccessWithToken:(NSString * _Nonnull)token packageId:(NSString * _Nonnull)packageId;
...
- (nonnull instancetype)initWithMatchId:(NSString * _Nullable)matchId token:(NSString * _Nullable)token type:(enum ViewType)type env:(enum Environment)env debug:(BOOL)debug OBJC_DESIGNATED_INITIALIZER;
